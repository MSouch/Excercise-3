...
const ChallengeQuestion = ({challenge, onComplete}) => {
  ...
  const [allowContinueFromExcellent, setAllowContinueFromExcellent] = useState(false)
  ...
  const handleSubmit = async (e) => {
    ...
    if (correct) {
      // Show 'Excellent!' feedback with a Continue button instead of auto-advance
      setAllowContinueFromExcellent(true)
    }
  }
      ...
      >
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[['A','B','C','D']
              .filter(k => k in challenge.options)
              .map(k => [k, challenge.options[k]])]
              .flat()
              .map(([key, option]) => (
              <motion.div
                key={key}
                ...
                <div className="flex justify-center space-x-4 mt-6">
                  ...
                  {isCorrect && (
                    <button
                      ...
                      disabled={!allowContinueFromExcellent}
                      className="bg-success-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-success-700 disabled:opacity-50 transition-colors"
                    >
                      Continue
                    </button>
                  )}
