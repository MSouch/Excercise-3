...
const NameCaptureForm = () => {
  ...
  const validateForm = () => {
    ...
    // Email removed for security; no validation
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
  const result = registerUser(formData.fullName.trim(), null)
      if (result.data.user) {
        navigate('/dashboard')
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        ...
        />
        <h2 className="text-2xl font-bold text-gray-900">Welcome to Communication Navigator</h2>
  <p className="text-gray-600">Enter your name to begin the training simulation</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Email collection removed intentionally */}
